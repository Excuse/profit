...
public class EditUserFrame extends JFrame implements ActionListener, WindowListener {
	...
	public void showFrame(User user, User previous) {
		...
		if (previous.IsAdmin()) {
			rankBox.setSelectedItem("Администратор");
		} else {
			rankBox.setSelectedItem("Пользователь");
		}
	...
	private EditUserFrame() {
		...
		editUserButton = new JButton("Сохранить изменения");
	...
	private void editUser(User previous, User current) throws NotBoundException, RemoteException {
		try {
			try {
				...
				JOptionPane.showMessageDialog(this, "Пользователь успешно отредактирован",
						"Редактирование пользователя", JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(this, "Пользователь не был отредактирован",
					"Редактирование пользователя", JOptionPane.ERROR_MESSAGE);
		}
	}
